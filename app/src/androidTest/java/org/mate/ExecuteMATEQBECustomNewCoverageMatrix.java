...
public class ExecuteMATEQBECustomNewCoverageMatrix {
    ...
    public void useAppContext() {
        MATE.log_acc("Starting QBE custom new coverage exploration...");
        try (final MATE ignored = new MATE()) {
            final QBEApplication app = new QBEApplication(Registry.getUiAbstractionLayer());
            final ExplorationStrategy<QBEState, QBEAction> explorationStrategy = new QBE<>(new QBEMatrixFactory().getCustomNewCoverageMatrix());
