...
public enum AccessibilityCheckPreset {
  ...
  public static Set<AccessibilityInfoHierarchyCheck>
    ...
    checks.add(new EditableContentDescInfoCheck());//as it is
    checks.add(new SpeakableTextPresentInfoCheck());//as it is
    if (preset == VERSION_1_0_CHECKS) {
      return checks;
    }

    checks.add(new ClickableSpanInfoCheck()); //as it is
    checks.add(new TouchTargetSizeInfoCheck()); //adapted
    //checks.add(new RedundantContentDescInfoCheck()); //as it is
    checks.add(new DuplicateClickableBoundsInfoCheck()); //adapted
