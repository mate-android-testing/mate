...
public class NewUniformRandomForAccessibility {
    ...
    public NewUniformRandomForAccessibility(DeviceMgr deviceMgr,
        ...
        this.algorithm = new RandomAlgorithmImpl_1(deviceMgr,packageName,guiModel,true);
    }

    public NewUniformRandomForAccessibility(RandomAlgorithm algorithm, boolean runAccChecks){
        currentActivityName="";
        this.algorithm = algorithm;
        this.deviceMgr = algorithm.getDeviceMgr();
        this.packageName = algorithm.getPackageName();
        this.guiModel = algorithm.getGuiModel();
        currentActivityName="";
        this.runAccChecks = runAccChecks;
    }
    ...
    public void startUniformRandomExploration(IScreenState selectedScreenState, long runningTime) {
        ...
        while (currentTime - runningTime <= MATE.TIME_OUT){
            try{
                ...
                //WidgetAction action = this.algorithm.getAExecutableActions(selectedScreenState);
                //execute this selected action
                //deviceMgr.executeAction(action);
                this.algorithm.executeAction(eventAction);
                ...
                if (currentPackageName!=null && currentPackageName.contains("com.android.packageinstaller")) {
                    currentPackageName = this.algorithm.handleAuth(currentPackageName);
                }
                ...
                if (!currentPackageName.equals(this.packageName)||numberOfActions>=MATE.RANDOM_LENGH) {
                    ...
                    if (this.runAccChecks) {
                        ...
                        if (newState || numberOfActions == 1) {
                            violationsAmountFounded = 0;

                            //TODO - fix the bug in runAccessibilityChecks
                            //violationsAmountFounded = RandomAccessibilityMethods.runAccessibilityChecks(state);
                            eventAction.increaseFitness(violationsAmountFounded);
                            eventAction.increaseWeight(1);
                        }
                    }
                }
        ...
        ResultsRandomExecution.printResultsNew(this.algorithm.getHistoric());
